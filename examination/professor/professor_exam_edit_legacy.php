...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRFToken($token)) {
        ...
        if ($title === '') {
            ...
            if (!empty($error)) {
                // keep user on page
            } elseif ($examId > 0 && examination_questions_mutations_locked($conn, 'regular', (int)$examId)) {
                $error = 'Questions are locked because this examination already has student attempts. Use the Examination wizard Questions step (read-only) or contact support. Destructive replace is blocked.';
            } else {
  ...
  <main class="dashboard-shell w-full max-w-none flex-1 min-w-0">
      ...
      <div class="prof-hero overflow-hidden">
        <div class="p-4 sm:p-5 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div class="flex flex-wrap items-center gap-2 shrink-0 lg:pt-0.5">
            <a href="professor_examinations" class="prof-btn-outline inline-flex items-center gap-1.5 px-3 py-2 text-xs sm:text-sm font-semibold">
    ...
    <form id="exam-edit-form" method="post" accept-charset="UTF-8" class="space-y-6 dash-anim delay-2" novalidate>
      ...
      <div class="flex flex-wrap items-center gap-2 pb-8 pt-2 dash-anim delay-3 border-t border-green-100 mt-2">
        ...
        <a href="professor_examinations" class="inline-flex items-center gap-1.5 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-600 hover:text-gray-900">Cancel</a>
      </div>
    </form>
