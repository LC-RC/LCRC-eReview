...
$uploadDir = dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'college';
if (!is_dir($uploadDir)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_task_id'])) {
    ...
    $taskPost = college_upload_fetch_task_for_student($conn, $postTid, (int) $uid);
    $can = $taskPost ? college_upload_can_student_submit($conn, (int) $uid, $taskPost) : ['ok' => false, 'error' => 'This task is not available.'];

    if (!$taskPost || empty($can['ok'])) {
        $_SESSION['error'] = (string) ($can['error'] ?? 'This task is closed or not available.');
        header('Location: college_upload_task?id=' . (int)$taskId);
        exit;
    }
    ...
    $record = college_upload_record_submission($conn, $taskId, (int) $uid, $relPath, (string) $f['name'], (int) $f['size']);
    if (empty($record['ok'])) {
        @unlink($dest);
        $_SESSION['error'] = (string) ($record['error'] ?? 'Could not save submission.');
        header('Location: college_upload_task?id=' . (int)$taskId);
        exit;
    }
...
$task = college_upload_fetch_task_for_student($conn, $taskId, (int) $uid);
if (!$task) {
...
$canSubmit = college_upload_can_student_submit($conn, (int) $uid, $task);
$open = !empty($canSubmit['ok']);
$history = college_upload_submission_history($conn, $taskId, (int) $uid);
$latest = college_upload_latest_submission($conn, $taskId, (int) $uid);
$resubLabel = college_upload_resubmission_label($conn, $taskId, (int) $uid);

$task['submission_id'] = $latest['submission_id'] ?? null;
$task['submitted_file'] = $latest['file_name'] ?? null;
$task['submitted_at'] = $latest['submitted_at'] ?? null;
$task['file_path'] = $latest['file_path'] ?? null;
...
$openAtRaw = trim((string) ($task['open_at'] ?? ''));
$openAtLabel = $openAtRaw !== '' ? date('M j, Y g:i A', strtotime($openAtRaw)) : '';
$closeLabel = date('M j, Y g:i A', strtotime((string) $task['deadline']));
$submitBlockCode = $open ? '' : (string) ($canSubmit['code'] ?? 'blocked');
$closedMessage = match ($submitBlockCode) {
    'upcoming' => 'This task opens on ' . ($openAtLabel !== '' ? $openAtLabel : 'the scheduled date') . '. Submission is not available yet.',
    'locked' => 'The deadline has passed. Contact your instructor if you need an exception.',
    'resubmit_blocked' => 'Resubmission requires professor approval before you can upload again.',
    'already_submitted' => 'You have already submitted for this task. Resubmission is not allowed.',
    default => 'Submission is not available for this task.',
};
$heroStatusClass = match ($windowState) {
    'open' => 'cut-open',
    'upcoming' => 'cut-upcoming',
    default => 'cut-closed',
};
$heroStatusIcon = match ($windowState) {
    'open' => 'bi-clock-history',
    'upcoming' => 'bi-calendar-event',
    default => 'bi-lock',
};
$heroStatusText = match ($windowState) {
    'upcoming' => 'Opens ' . ($openAtLabel !== '' ? $openAtLabel : 'soon'),
    'open' => 'Closes ' . $closeLabel,
    'locked' => 'Closed ' . $closeLabel,
...
<head>
  ...
  <style>
    ...
    .cut-upcoming { color: #1e40af; background: #eff6ff; border-color: #bfdbfe; }
...
<body class="font-sans antialiased<?php echo !empty($examinationStudentBodyClass) ? ' ' . h($examinationStudentBodyClass) : ''; ?>">
  <?php include __DIR__ . '/college_student_sidebar.php'; ?>
    ...
    <?php
      ...
      $cpPageTitle = (string)($task['title'] ?? 'Upload task');
      $cpPageIcon = 'bi-file-earmark-arrow-up';
      $cpPageSubtitle = 'Follow the instructions, then submit your file before the deadline.';
      $cpPageActionHtml = '<span class="cp-status ' . h($heroStatusClass) . '">'
        . (!empty($heroStatusIcon) ? '<i class="bi ' . h($heroStatusIcon) . '"></i> ' : '')
        . h($heroStatusText) . '</span>';
      ...
      <aside class="lg:col-span-5 xl:col-span-4 min-w-0">
        <div class="cut-sticky-panel space-y-4">
          <div class="cp-dash-panel cut-panel">
            ...
            <div class="cp-dash-panel__body cut-panel-body">
              <div class="cut-side-meta m-0 mb-4 pb-3 border-b border-slate-100">
                ...
                <?php else: ?>
                  <strong>Open:</strong> Immediately when published<br>
                <?php endif; ?>
                <strong>Closes:</strong> <?php echo h($closeLabel); ?>
              </div>
              ...
              <?php if ($resubPolicy !== 'disabled' || $resubLabel !== 'No request'): ?>
                <div class="rounded-lg border border-slate-200 bg-slate-50/80 p-3 mt-4 mb-4">
                  <p class="text-xs font-extrabold text-slate-600 uppercase tracking-wide m-0 mb-1">Resubmission</p>
                  <p class="text-sm font-semibold text-slate-800 m-0"><?php echo h($resubLabel); ?></p>
                  <?php if ($resubPolicy === 'allowed'): ?>
                    <p class="text-xs text-slate-500 m-0 mt-1">You may submit a new file while the task is open.</p>
                  <?php elseif ($resubLabel === 'Pending'): ?>
                    <p class="text-xs text-slate-500 m-0 mt-1">Waiting for professor approval.</p>
                  <?php elseif ($resubLabel === 'Approved'): ?>
                    <p class="text-xs text-emerald-700 m-0 mt-1">You may upload a new version.</p>
                  <?php endif; ?>
                </div>
              <?php endif; ?>
              ...
              <?php if (!empty($history)): ?>
                <div class="mb-4">
                  <p class="text-xs font-extrabold text-slate-600 uppercase tracking-wide m-0 mb-2">Submission history</p>
                  <ul class="space-y-2 m-0 p-0 list-none">
                    <?php foreach ($history as $hist):
                      $hid = (int) ($hist['submission_id'] ?? 0);
                      $hNum = (int) ($hist['submission_number'] ?? 1);
                      $hLatest = !empty($hist['is_latest']);
                      $hReview = ucfirst((string) ($hist['review_status'] ?? 'submitted'));
                      $hView = $hid > 0 ? 'college_upload_file?s=' . $hid : '';
                    ?>
                    <li class="rounded-lg border <?php echo $hLatest ? 'border-emerald-200 bg-emerald-50/60' : 'border-slate-200 bg-white'; ?> p-3">
                      <div class="flex flex-wrap items-center justify-between gap-2">
                        <span class="text-sm font-bold text-slate-900">Submission #<?php echo $hNum; ?></span>
                        <?php if ($hLatest): ?>
                          <span class="cut-status cut-done text-xs"><i class="bi bi-star-fill"></i> Latest</span>
                        <?php else: ?>
                          <span class="text-xs font-semibold text-slate-500"><?php echo h($hReview); ?></span>
                        <?php endif; ?>
                      </div>
                      <p class="text-xs text-slate-600 m-0 mt-1"><?php echo h(date('M j, Y g:i A', strtotime((string) ($hist['submitted_at'] ?? 'now')))); ?></p>
                      <p class="text-sm font-medium text-slate-800 m-0 mt-1 break-words"><?php echo h((string) ($hist['file_name'] ?? '')); ?></p>
                      <?php if ($hView !== ''): ?>
                        <a href="<?php echo h($hView); ?>" target="_blank" rel="noopener" class="cut-preview-link text-xs mt-2 inline-flex"><i class="bi bi-box-arrow-up-right"></i> View file</a>
                      <?php endif; ?>
                    </li>
                    <?php endforeach; ?>
                  </ul>
              ...
              <form method="post" enctype="multipart/form-data" class="space-y-3" id="cut-upload-form">
                ...
                <button type="submit" class="cut-submit">
                  ...
                  <?php echo !empty($task['submission_id']) ? 'Submit again' : 'Upload file'; ?>
                </button>
              </form>
              <?php else: ?>
              <p class="text-sm text-slate-600 font-medium m-0"><?php echo h($closedMessage); ?></p>
              <?php if (!empty($history)): ?>
                <p class="text-xs text-slate-500 m-0 mt-2">Your previous submissions remain available above.</p>
              <?php endif; ?>
              <?php endif; ?>
            </div>
          </div>
          ...
          <div class="flex flex-wrap gap-2 dash-anim delay-2">
            <a href="college_uploads" class="cp-btn w-full sm:w-auto">
