...
<body class="font-sans antialiased admin-app admin-test-bank-page">
  ...
  <div class="quiz-admin-hero rounded-xl px-5 py-5 mb-5">
    ...
    <h1 class="text-2xl font-bold text-gray-100 m-0 flex items-center gap-2 flex-wrap">
      ...
      Test Bank â€” <span class="admin-subject-text admin-subject-text--testbank"><?php echo h($subject['subject_name']); ?></span>
    </h1>
  ...
  <form method="get" action="admin_test_bank" class="quiz-admin-filter quiz-admin-table-shell rounded-xl px-4 py-3 mb-4 flex flex-wrap items-end gap-3">
    ...
    <div class="flex-1 min-w-[200px]">
      ...
      <input type="search" id="tb-search-q" name="q" value="<?php echo h($searchQ); ?>" placeholder="Search title or descriptionâ€¦" class="input-custom w-full" autocomplete="off">
    </div>
    <div class="flex flex-wrap gap-2">
                  ...
                  <tr class="quiz-admin-row">
                    <td class="px-4 sm:px-5 py-2.5 sm:py-3">
                      ...
                      <?php if (!empty($row['description'])): ?>
                        <p class="text-xs sm:text-sm text-gray-500 mt-0.5 line-clamp-2"><?php echo h(mb_substr($row['description'], 0, 60)); ?><?php echo mb_strlen($row['description']) > 60 ? 'â€¦' : ''; ?></p>
                      <?php endif; ?>
                    </td>
                    <td class="px-4 sm:px-5 py-2.5 sm:py-3 whitespace-nowrap">
                      ...
                      <?php else: ?><span class="text-gray-600">â€”</span><?php endif; ?>
                    </td>
                    <td class="px-4 sm:px-5 py-2.5 sm:py-3 whitespace-nowrap">
                    ...
                    <td class="px-4 sm:px-5 py-2.5 sm:py-3 text-xs sm:text-sm text-gray-400 whitespace-nowrap"><?php echo $row['updated_at'] ? date('M j, Y', strtotime($row['updated_at'])) : 'â€”'; ?></td>
                    <td class="px-4 sm:px-5 py-2.5 sm:py-3">
