...
$pageTitle = 'Preboard Review â€” ' . ($attempt['full_name'] ?? 'Student');
$adminBreadcrumbs = [
...
<body class="font-sans antialiased admin-app admin-preboards-review-page">
  ...
  <div class="quiz-admin-hero rounded-xl px-5 py-5 mb-5">
    ...
    <p class="text-gray-400 mt-2 mb-0 max-w-3xl text-sm sm:text-base">
      <?php echo h($attempt['full_name'] ?? ''); ?> â€” <?php echo h($attempt['subject_name'] ?? ''); ?>, Set <?php echo h($attempt['set_label'] ?? ''); ?>
      <?php if (!empty($attempt['set_title'])): ?> Â· <?php echo h($attempt['set_title']); ?><?php endif; ?>
    </p>
  </div>
  ...
  <div class="flex flex-wrap justify-between items-center gap-3 mb-5">
    ...
    <?php if (!$isSubmitted): ?>
      <span class="px-3 py-1.5 rounded-full text-sm font-semibold bg-amber-500/15 text-amber-300 border border-amber-500/30">In progress â€” answers may be incomplete</span>
    <?php endif; ?>
  </div>

  <div class="pb-review-summary mb-5">
    <div class="pb-review-summary-item"><strong><?php echo $isSubmitted ? preboards_format_score($score) : 'â€”'; ?></strong><span>Score</span></div>
        ...
        <tbody>
          ...
          ?>
            <tr class="quiz-admin-row <?php echo $current ? 'opacity-100' : ''; ?>">
              ...
              <td class="px-5 py-2 text-gray-300"><?php echo ($h['status'] ?? '') === 'submitted' ? preboards_format_score(isset($h['score']) ? (float) $h['score'] : null) : 'â€”'; ?></td>
