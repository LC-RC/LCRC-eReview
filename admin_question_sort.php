<?php
/**
 * Admin: automated question sorting from .docx â€” parse, group by trailing (Topic), export JSON/HTML/DOCX.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['download'])) {
    ...
    if ($kind === 'parse_trace') {
        ...
        if (!is_array($tr)) {
            ...
            echo 'No parse trace in this session. Upload the .docx again and enable â€œCapture parser traceâ€.';
            exit;
        }
        $export = [
            'meta' => $data['meta'] ?? [],
    ...
    if ($kind === 'html') {
        ...
        echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Grouped â€” ' . $esc($baseName) . '</title>';
...
<style>
    ...
    /* Deploy modal â€” viewport-centered via x-teleport="body" + flex overlay */
...
<body class="font-sans antialiased admin-app admin-question-sort-page">
    ...
    <form method="post" enctype="multipart/form-data" class="space-y-4" id="ereviewQsortForm">
      ...
      <div class="flex flex-col gap-3">
        <label class="inline-flex items-start gap-2.5 cursor-pointer text-sm text-gray-400 max-w-2xl">
          ...
          <span><strong class="text-gray-300">Capture parser trace</strong> â€” logs every paragraph decision (list metadata, <code class="text-gray-500">in_choices</code>, split-after-choices flags). Use when a question sticks to the wrong item or topic. After upload, open <a class="text-sky-400 hover:underline" href="admin_question_sort?qsort_debug=1">Parser debug</a> or download <code class="text-gray-500">.parse-trace.json</code>.</span>
        </label>
  ...
  <div x-data="adminQuestionSortResults()" class="space-y-4">
    <div class="quiz-admin-table-shell rounded-xl overflow-hidden">
      <div class="preweek-section-toolbar px-4 sm:px-5 py-3.5 flex flex-wrap items-center gap-3 border-b border-white/[0.08]">
        ...
        if ($__qc > 0 || $__gpc > 0):
        ?>
        <span class="text-sm text-gray-300 font-semibold tabular-nums" title="Parsed counts for this document"><?php echo $__qc; ?> questions<?php if ($__gpc > 0): ?> Â· <?php echo $__gpc; ?> general problem<?php echo $__gpc === 1 ? '' : 's'; ?><?php endif; ?></span>
        <?php endif; ?>
        ...
        <div class="w-full sm:flex-1 sm:max-w-md sm:ml-auto min-w-0">
          ...
          <div class="relative">
            ...
            <input type="search" id="ereview_qsort_filter" x-model.debounce.200ms="filter" placeholder="Topic, general problem, or questionâ€¦" class="input-custom w-full pl-10" autocomplete="off">
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="quiz-admin-table-shell rounded-xl overflow-hidden ereview-qsort-gp-shell" x-show="generalProblems.length > 0" x-cloak>
      ...
      <div class="ereview-qsort-topic-body border-t border-white/[0.06]" x-show="gpOpen" x-cloak>
        <p class="text-xs text-gray-500 m-0 px-4 pt-3 pb-1">These blocks were split out of a preceding itemâ€™s choices. Later numbered questions (for example 205â€“212) refer to this context.</p>
        ...
        >
          ...
          <div class="ereview-deploy-panel__body">
            ...
            <div class="ereview-deploy-field-group ereview-deploy-target-row">
              ...
              >
                <option value="">Create new topical quizâ€¦</option>
                <template x-for="qz in existingQuizzes" :key="'qzopt-' + qz.quiz_id">
                  <option :value="String(qz.quiz_id)" x-text="(qz.title || 'Untitled') + ' Â· ' + qz.question_count + ' question' + (qz.question_count === 1 ? '' : 's')"></option>
                </template>
              </select>
              <p class="ereview-deploy-quiz-load" x-show="deployQuizzesLoading" x-cloak><i class="bi bi-arrow-repeat animate-spin"></i> Loading quizzes for this subjectâ€¦</p>
          ...
          <footer class="ereview-deploy-footer">
            ...
            <button type="button" @click="submitDeploy()" :disabled="deployBusy || deploySubjects.length === 0" class="px-5 py-2.5 rounded-lg font-semibold bg-violet-600 text-white hover:bg-violet-500 transition inline-flex items-center justify-center gap-2 min-w-[7.5rem] shadow-lg shadow-violet-900/35 disabled:opacity-45 disabled:pointer-events-none">
              ...
              <span x-text="deployBusy ? (deployTargetQuizId ? 'Appendingâ€¦' : 'Deployingâ€¦') : (deployTargetQuizId ? 'Append questions' : 'Deploy')"></span>
            </button>
          </footer>
        </div>
      </div>
  ...
  <div class="quiz-admin-table-shell rounded-xl overflow-hidden mb-5 ereview-qsort-debug-panel">
    <div class="preweek-section-toolbar px-4 sm:px-5 py-3.5 border-b border-white/[0.08]">
      ...
      <p class="text-xs text-gray-500 m-0 mt-1 max-w-3xl">Rows match <code class="text-gray-400">word/document.xml</code> body paragraphs in order. Look for question 4â€™s last choice, then the next row: if <code class="text-gray-400">action</code> is <strong class="text-gray-300">append_while_in_choices_not_choice_shape</strong>, the splitter did not run. Check <code class="text-gray-400">after_choices.split_para</code> (should be <code class="text-gray-400">true</code> for a new item after <code class="text-gray-400">a.â€“d.</code>).</p>
    </div>
    <div class="p-4 sm:p-5">
      <?php if ($parseTraceForView === null): ?>
      ...
      <?php else: ?>
        ...
        <h3 class="text-sm font-bold text-gray-300 mt-0 mb-2">Questions outline (number â†’ topic)</h3>
          ...
          <table class="w-full text-left text-xs text-gray-300 border-collapse">
            ...
            <tbody>
              ...
              <tr class="border-t border-white/[0.06]">
                ...
                <td class="p-2"><?php echo h((string)($row['topic'] ?? 'â€”')); ?></td>
          ...
          <table class="w-full text-left text-xs text-gray-300 border-collapse min-w-[900px]">
            ...
            <tbody>
              ...
              <?php
                ...
                if ($act === 'summary') {
                    echo '<tr class="bg-amber-900/20 border-t border-amber-500/30"><td class="p-2 font-mono" colspan="8">summary â€” paragraphs: ' . h((string)($row['total_paragraphs'] ?? '')) . ', blocks: ' . h((string)($row['total_question_blocks'] ?? '')) . '</td></tr>';
                    continue;
                }
                $list = $row['list'] ?? [];
  ...
  (function () {
    ...
    if (form && btn) {
      form.addEventListener('submit', function () {
        ...
        if (sp) sp.textContent = 'Processingâ€¦';
      });
    }
  })();
  </script>
