<?php
/**
 * Preweek materials admin â€” same UX and upload rules as admin_materials
 * (video URL or file upload, handout file, allow download toggle, search/filter).
 * Data: preweek_topics â†’ preweek_videos / preweek_handouts (under preweek_units).
 */
        ...
        if (!$errors) {
            if ($uploadType === 'file') {
                ...
                if ($hasFile) {
                    ...
                    $errors[] = $videoId > 0
                        ? 'Choose a new video file to replace the current one, or switch to â€œLinkâ€ and paste a URL.'
...
$searchQ = trim($_GET['q'] ?? '');
// No `type` in URL â†’ default to videos-only. Explicit `type=` means "Videos & handouts".
...
$pageTitle = 'Preweek materials â€” ' . h($topicTitle);
...
$preweekUploadMax = ini_get('upload_max_filesize') ?: 'â€”';
$preweekPostMax = ini_get('post_max_size') ?: 'â€”';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="font-sans antialiased admin-app admin-materials-page admin-preweek-materials-page">
  ...
  <div class="quiz-admin-hero rounded-xl px-5 py-5 mb-4">
    <h1 class="text-2xl font-bold text-gray-100 m-0 flex flex-wrap items-center gap-2">
      ...
      Materials <span class="text-gray-500 font-semibold">â€”</span> <span class="text-amber-200 font-semibold"><?php echo h($topicTitle); ?></span>
    </h1>
  ...
  <form method="get" action="admin_preweek_materials" class="quiz-admin-filter quiz-admin-table-shell rounded-xl px-4 py-3 mb-3 flex flex-wrap items-end gap-3">
    ...
    <div class="w-full sm:flex-1 sm:min-w-[200px] sm:max-w-lg">
      ...
      <div class="relative">
        ...
        <input type="search" id="mat-search-q" name="q" value="<?php echo h($searchQ); ?>" placeholder="Title or filenameâ€¦" class="input-custom w-full pl-10" autocomplete="off">
      </div>
    </div>
            ...
            <tbody>
              <?php mysqli_data_seek($handouts, 0); while ($h = mysqli_fetch_assoc($handouts)):
                ...
                <tr class="materials-data-row">
                  ...
                  <td class="px-4 py-3 align-top">
                    <div class="text-gray-200 text-sm"><?php echo !empty($h['file_name']) ? h($h['file_name']) : 'â€”'; ?></div>
                  </td>
                  <td class="px-4 py-3 align-top">
                    <?php if ($fp !== ''): ?>
        ...
        <form id="preweekVideoForm" method="post" enctype="multipart/form-data" action="<?php echo $preweekMatAction; ?>" novalidate>
          ...
          <div class="space-y-3">
            <div>
              ...
              <input type="text" name="video_title" id="videoTitleInput" class="input-custom w-full" maxlength="255" placeholder="e.g. Preweek intro â€” overview" autocomplete="off">
            ...
            <div id="preweekAddUrlWrap">
              ...
              <input type="url" name="video_url" id="preweekVideoUrl" class="input-custom w-full" placeholder="https://www.youtube.com/watch?v=â€¦" inputmode="url" autocomplete="url">
            </div>
            ...
            <p id="preweekVideoAutosaveHint" class="preweek-field-hint text-gray-500 mt-2 mb-0 max-w-xl">
              Title, source, and URL are saved in this browser until the video is added successfully or you discard the draft. (Uploaded files cannot be rememberedâ€”select the file again after reopening.)
            </p>
        ...
        <form id="preweekHandoutForm" method="post" enctype="multipart/form-data" action="<?php echo $preweekMatAction; ?>" novalidate>
          ...
          <div class="space-y-3">
            <div>
              ...
              <input type="text" name="handout_title" id="handoutTitleInput" class="input-custom w-full" maxlength="255" placeholder="Optional â€” for a single file, defaults to file name" autocomplete="off">
            </div>
            ...
            <div>
              ...
              <p class="preweek-field-hint">PDF, Word, PowerPoint, Excel, TXT â€” multiple files or one folder. Each file becomes one handout. Max files per request is limited by your PHP <code class="text-gray-400">max_file_uploads</code> setting.</p>
            </div>
            <div class="flex items-start gap-3 rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2.5">
        ...
        <form id="preweekVideoEditForm" method="post" enctype="multipart/form-data" action="<?php echo $preweekMatAction; ?>">
          ...
          <div class="space-y-3">
            ...
            <div>
              ...
              <p class="preweek-field-hint">To replace an uploaded file, choose â€œUpload fileâ€ and pick a new video. To change only the title or link, adjust fields and save.</p>
            </div>
...
<script>
  (function () {
    ...
    function setLoading(btn, loading) {
      ...
      if (label) {
        label.textContent = loading ? 'Please waitâ€¦' : def;
      }
